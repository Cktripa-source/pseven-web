...
function Navbar({ cartCount }) {
  ...
  <div className="md:flex justify-between items-center px-6 py-1 bg-black text-sm text-white transition duration-300">
    <div className="p-2 text-center md:text-left"> {/* Reduced padding here */}
    ...
    <div className="flex space-x-6 mt-2 md:mt-0 "> {/* Reduced margin-top here */}
      <Link to="/login" className=" text-white py-1.5 px-6 rounded-lg hover:text-red-500  transition duration-300 flex items-center space-x-2">
        <FaSignInAlt className="h-5 w-5 text-white hover:text-red-500 " />
        <span>Login</span>
      </Link>
      <Link to="/register" className=" text-white py-1.5 px-6 rounded-lg hover:text-red-500  transition duration-300 flex items-center space-x-2">
        <FaUserPlus className="h-5 w-5 text-white hover:text-red-500  " />
        <span>Register</span>
      </Link>
    </div>
  </div>
  ...
  <div className="flex mt-2 justify-between items-center px-6 py-1 bg-gray-900 transition duration-300 ease-in-out flex-wrap"> {/* Reduced padding */}
    <div className="flex items-center space-x-4">
      <Link to="/" className="flex items-center space-x-2">
        <img src={Logo} alt="Logo" className="h-10 rounded-full transition duration-300 hover:scale-110 transform" /> {/* Reduced logo size */}
        <span className="font-bold text-lg transition duration-300 hover:text-gray-400"> {/* Reduced font size */}
    ...
    <div className="flex items-end justify-between space-x-6 w-full max-w-xl mx-auto mt-2 lg:mt-0">
      <div className="relative w-full lg:w-1/2">
        <input type="text" placeholder="Search Product ........." className="py-1.5 px-4 rounded-lg bg-gray-800 text-white placeholder-gray-400 focus:outline-none w-full transition duration-300 ease-in-out" /> {/* Reduced padding */}
  ...
  <div className="hidden md:flex justify-center items-center  py-1 transition duration-300 ease-in-out"> {/* Reduced padding */}
  ...
  <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} 
    className="lg:hidden md:hidden py-0 px-4 bg-white rounded-lg hover:bg-red-500 
